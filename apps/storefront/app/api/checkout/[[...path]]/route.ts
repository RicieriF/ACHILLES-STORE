...
export async function PATCH(request: Request, context: Context) {
  return forward("PATCH", context, await request.text());
}
...
async function forward(
  ...
  if (!path.every((segment) => /^[a-zA-Z0-9_-]{1,128}$/.test(segment)))
    return NextResponse.json(
      { message: "Rota de checkout inválida" },
      { status: 400 },
    );
  try {
    ...
    const response = await fetch(
      `${commerceUrl}/achilles/store/checkout${path.length ? `/${path.join("/")}` : ""}`,
      init,
    );
